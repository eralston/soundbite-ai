...
function readPendingSessions(
  ...
  if (groupRoute == null) {
    return SessionsService.readPendingAsync(orgRoute, page);
  } else {
    return SessionsService.readGroupPendingAsync(orgRoute, groupRoute, page);
  }
}

/** Display an expandable list of sessions that are pending for the given org or group */
export const PendingSessionsWidget: React.FC<IProps> = observer(
  (props: IProps) => {
    ...
    const icon = props.icon ?? faCalendarDay;
    ...
    async function getTitle(group?: Group): Promise<string> {
      ...
      if (props.orgRoute) {
        ...
        if (org) {
          return `${org.details.name} Scheduled`;
        }
      }
    ...
    const onFirstLoad = async (
      ...
      const ret = await readPendingSessions(orgRoute, groupRoute, page);
    ...
    const Body: React.FC = () => {
      let emptyMsg = "No Scheduled Sessions";
      if (isCreateEnabled) {
        emptyMsg +=
          ". Create a new scheduled or recurring session and it will appear here.";
      }
      if (count === 0) {
        return (
            ...
            <Row className="align-items-center">
              ...
              <Col className="text-right" xs="4" md="6">
                <Button
                  ...
                  onClick={onRefresh}
                  size="sm"
                  title="Refresh Scheduled Sessions"
                >
                  <span className="btn-inner--icon">
